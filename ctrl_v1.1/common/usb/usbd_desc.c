...
{
	USBD_GetString((uint8_t *)(speed == USB_OTG_SPEED_HIGH ? USBD_PRODUCT_HS_STRING : USBD_PRODUCT_FS_STRING), USBD_StrDesc, length);
	return USBD_StrDesc;
}
...
{
	USBD_GetString((uint8_t *)(speed == USB_OTG_SPEED_HIGH ? USBD_CONFIGURATION_HS_STRING : USBD_CONFIGURATION_FS_STRING), USBD_StrDesc, length);
	return USBD_StrDesc;
}

uint8_t *USBD_USR_InterfaceStrDescriptor(uint8_t speed, uint16_t *length)
{
	USBD_GetString((uint8_t *)(speed == USB_OTG_SPEED_HIGH ? USBD_INTERFACE_HS_STRING : USBD_INTERFACE_FS_STRING), USBD_StrDesc, length);
	return USBD_StrDesc;
}
