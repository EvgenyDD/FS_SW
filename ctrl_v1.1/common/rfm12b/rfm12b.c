...
#define CS_H GPIOD->BSRRL = (1 << 2)
#define CS_L GPIOD->BSRRH = (1 << 2)
...
#define rf12_hdr1 rf12_buf[1]	 /// pointer to 1st header byte in rf12_buf (CTL + DESTINATIONID)
#define rf12_hdr2 rf12_buf[2]	 /// pointer to 2nd header byte in rf12_buf (ACK + SOURCEID)
#define rf12_len rf12_buf[3]	 /// RF12 length byte in rf12_buf.
#define rf12_data (rf12_buf + 4) /// first RF12 data byte in rf12_buf.
...
bool rfm12b_rx_complete(void)
{
	...
	if(rxstate == TXIDLE)
	{
		...
		rxfill = rf12_len = 0;
		rf12_crc = 0xFFFF;
		if(networkID != 0) rf12_crc = _crc16_update(0xFFFF, networkID);
		rxstate = TXRECV;
...
void EXTI15_10_IRQHandler(void)
{
	...
	{
		EXTI_ClearITPendingBit(EXTI_Line12);
		if(RFM_IRQ_GET() == false) rfm12b_irq_handler();
	}
}
...
void rfm12b_irq_handler(void)
{
	...
	if(rxstate == TXRECV)
	{
		...
		if(rxfill >= (uint8_t)(rf12_len + 6) || rxfill >= RF_MAX) rfm12b_trx_2b(RFM12B_IDLE_MODE);
	}
	else
	{
		uint8_t out;
		...
		else
		{
			...
			case TXDONE: rfm12b_trx_2b(RFM12B_IDLE_MODE);
			// fall through
			default: out = 0xAA;
			}
		}
