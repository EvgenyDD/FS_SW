...
static uint16_t get_approximation(float servo, float adc)
{
		...
		for(uint32_t i = 0; i < SERVO_COUNT; i++)
		{
			if(servo_move[i].mode == MODE_MOVE)
			{
				...
				else
				{
					...
					{
						debug_rf("%d>%d|%d|%d|%d|%d|%d|(%d)", pos, adc_val.s_pos[0], adc_val.s_pos[1], adc_val.s_pos[2], adc_val.s_pos[3], adc_val.s_pos[4], adc_val.s_pos[5], adc_val.s_pos[6]);
