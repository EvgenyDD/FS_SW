...
#define AIR_PROTO_DEBUG_LEVEL 0
#endif
...
inline static void regenerate_timeout_slave_tx_lost(void) { timeout_slave_tx_lost = system_time_ms + TIMEOUT_SLAVE_TX_LOST; }
...
static storage_t storage[NODES_COUNT] = {0};
...
static bool waitForAck(uint8_t dest_node_id)
{
	...
	int32_t mark_prev = 0;
	prof_mark(&mark_prev);
	const uint32_t time_limit = ACK_TIMEOUT_MS * SYSTICK_IN_MS;
	for(;;)
	{
...
void air_protocol_poll(void)
{
	...
	case AIR_PROTOCOL_STATE_RX:
		poll_rx();
		if(timeout_slave_tx_lost < system_time_ms)
		{
	...
	case AIR_PROTOCOL_STATE_SELECT_NEXT_NODE:
		...
		timeout_idle = system_time_ms + TIMEOUT_IDLE_MS;
	...
	case AIR_PROTOCOL_STATE_IDLE:
		...
		if(timeout_idle < system_time_ms)
		{
			change_state(AIR_PROTOCOL_STATE_SACK);
		}
		return;
	...
	case AIR_PROTOCOL_STATE_RX:
		poll_rx();
		if(to_counter_master + MASTER_HB_TIMEOUT < system_time_ms)
		{
	...
	case AIR_PROTOCOL_STATE_TX:
	{
		...
		to_counter_master = system_time_ms + MASTER_HB_TIMEOUT;
		change_state(AIR_PROTOCOL_STATE_RX);
	}
		return;
#endif
...
{
	if(iterator < NODES_COUNT) storage[iterator].dest_id = node_id;
}
