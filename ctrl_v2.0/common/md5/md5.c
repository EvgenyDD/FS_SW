...
static uint32_t S[] = {7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 5, 9, 14, 20, 5, 9, 14, 20, 5, 9, 14, 20, 5, 9, 14, 20,
...
static uint8_t PADDING[] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
 ...
 */
void md5_update(md5_ctx *ctx, const uint8_t *input_buffer, size_t input_len)
{
	uint32_t input[16];
	...
	for(unsigned int i = 0; i < input_len; ++i)
	{
		...
		if(offset % 64 == 0)
		{
			for(unsigned int j = 0; j < 16; ++j)
			{
				...
				// The local variable `input` our 512-bit chunk separated into 32-bit words we can use in calculations
...
void md5_finalize(md5_ctx *ctx)
{
	...
	for(unsigned int i = 0; i < 4; ++i) // Move the result into digest (convert from little-endian)
	{
